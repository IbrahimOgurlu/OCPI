...
using WattReise.OCPI.Data;
using WattReise.OCPI.Services;
using WattReise.OCPI.Clients;
...
builder.Services.AddSwaggerGen(c => {
    c.SwaggerDoc("v1", new() { Title = "WattReise OCPI API", Version = "v1" });
});
...
{
    app.UseSwagger(); 
    app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "WattReise OCPI v1")); // Swagger UI'ı etkinleştirir.
}
